...
DOUT_PIN = 21  # Define your DOUT pin for HX711
PD_SCK_PIN = 20  # Define your PD_SCK pin for HX711
...
def triac_control(delayValue):
    while True:
        sensorValue = GPIO.input(inputPin)
        if sensorValue == GPIO.HIGH:
            GPIO.output(triacPulse, GPIO.LOW)
            time.sleep(delayValue / 1_000_000)  # Convert microseconds to seconds
            GPIO.output(triacPulse, GPIO.HIGH)
        time.sleep(0.01)  # Add a small delay to prevent excessive CPU usage

def start_threads(delayValue):
    global triac_thread
    triac_thread = threading.Thread(target=triac_control, args=(delayValue,))
    ...
    return triac_thread, relay_thread, sensor_thread
